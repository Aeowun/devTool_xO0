...
from core.protocol import RelayError, CdpDisconnected, DEFAULT_URL
from adb.adb_client import run_adb, choose_device
...
(() => {
  ...
  const value = target.tagName === "TEXTAREA" || target.tagName === "INPUT" ? target.value : target.innerText;
  return {found:true, value:value || "", tag:target.tagName, editable:!!target.isContentEditable};
})()"""
...
(() => {
  ...
  const busy=!!document.querySelector("[data-is-streaming='true'],button[data-testid='stop-button'],.result-streaming");
  const continuation=document.querySelector("button[aria-label*='Continue'], button[class*='continue']");
  if (continuation && visible(continuation)) { continuation.click(); }
...
(() => {
  ...
  const target = document.getElementById("prompt-textarea");
  if (!target) return {found:false, value:""};
...
(() => {
  ...
  if (btn && visible(btn) && !btn.disabled && btn.getAttribute("aria-disabled") !== "true") {
     return {count: 1, labels: [(btn.getAttribute("aria-label") || btn.textContent || "").trim()]};
  }
...
(() => {
  ...
  return {clicked: false, reason: "Send button not found or not ready"};
})()"""

_MESSAGES_JS = r"""/* relay:capture-assistant-beacons */
(() => {
  const visible = e => { const r=e.getBoundingClientRect(), s=getComputedStyle(e);
    return r.width>0 && r.height>0 && s.display!=="none" && s.visibility!=="hidden"; };
  
  const turns = [...document.querySelectorAll("article, [data-testid*='message'], [class*='message']")];
  
  return turns.map((turn, index) => {
    const content = turn.querySelector(".markdown, .prose, [data-assistant-markdown]");
    const text = (content ? content.innerText : turn.innerText || "").trim();
    const id = turn.getAttribute("data-message-id") || turn.id || `turn-${index}`;
    const streaming = !!turn.querySelector(".result-streaming, .streaming, [class*='streaming']");
    
    return { id, text, streaming, beacon: false };
    ...
    *,
    ...
    if logger:
        logger.record(
            ...
            baseline_ids_size=len(transaction.baseline_ids),
            baseline_texts_size=len(transaction.baseline_texts)
        )
    try:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            preferred = [
                ...
                if any(h in str(target.get("url", "")).lower()
                       for h in ("chat.openai.com", "chatgpt.com"))
    ...
    try:
        try:
            ...
            run_adb(["shell", "am", "start", "-a", "android.intent.action.VIEW", "-d", url],
                    adb_path, selected)
            target = discover_cdp_target(port, url, min(timeout_s, 15.0))
        from urllib.parse import urlsplit, urlunsplit

        ws = urlsplit(target["webSocketDebuggerUrl"])
        local_ws = urlunsplit(("ws", "127.0.0.1", f":{port}", ws.path, ws.query))
        # Fixed the urlunsplit call above, let's be more precise
        local_ws = f"ws://127.0.0.1:{port}{ws.path}"
        if ws.query:
             local_ws += f"?{ws.query}"
...
def ensure_desktop_cdp(
    ...
    """Verify that a Chrome CDP endpoint is already reachable; start it if not."""
    try:
        _json_http(port, "/json/version", host)
        return
    except RelayError:
        # If it's not reachable and it's a local host, attempt to auto-launch.
        if host in ("127.0.0.1", "localhost"):
            chrome_path = _find_chrome()
            user_data_dir = os.path.join(os.environ.get("LOCALAPPDATA", ""), "Google", "Chrome", "Relay Data")
            cmd = [
                chrome_path,
            ...
            try:
                ...
                # Wait for Chrome to initialize the CDP endpoint
                ...
                raise RelayError(f"Unable to auto-start Chrome: {exc}") from exc
...
def connect_desktop_cdp(
    ...
    ws = urlsplit(target["webSocketDebuggerUrl"])
    local_ws = f"ws://{host}:{port}{ws.path}"
    if ws.query:
        local_ws += f"?{ws.query}"
